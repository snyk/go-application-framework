...
var Channel = make(chan ProgressParams, 10000)
var CancelProgressChannel = make(chan ProgressToken, 10000)

type Tracker struct {
	channel              chan ProgressParams
	cancelChannel        chan ProgressToken
	token                ProgressToken
	cancellable          bool
...
func NewTestTracker(channel chan ProgressParams, cancelChannel chan ProgressToken) *Tracker {
	return &Tracker{
		channel:       channel,
...
func (t *Tracker) GetToken() ProgressToken {
	return t.token
}

func newProgressParams(title, message string, cancellable, unquantifiableLength bool) ProgressParams {
	id := uuid.New().String()
	percentage := 1
...
func (t *Tracker) send(progress ProgressParams) {
	if progress.Token == "" {
