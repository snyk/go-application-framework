...
func TestAuthStatusFormat(t *testing.T) {
	assert.Contains(t, AuthStatus{OK: true, Identity: "user@snyk.io"}.format(), "Authenticated as: user@snyk.io")

	failed := AuthStatus{ErrorMessage: "error fetching user data: ... Authentication error"}.format()
	assert.Contains(t, failed, "Failed to verify authentication")
	assert.Contains(t, failed, "Error: error fetching user data: ... Authentication error")
}
