...
func getListOfShimFindings(input []workflow.Data, debugLogger *zerolog.Logger) (findings []*testapi.TestResult, remainingData []workflow.Data) {
	findings = []*testapi.TestResult{}
	remainingData = []workflow.Data{}

	for i := range input {
		if !strings.HasPrefix(input[i].GetContentType(), content_type.LOCAL_SHIM_FINDING_MODEL) {
			remainingData = append(remainingData, input[i])
			continue
		}
		debugLogger.Info().Msgf("[%s] Handling findings model", input[i].GetIdentifier().String())
		var localFindingsModel testapi.TestResult
		localFindingsBytes, ok := input[i].GetPayload().(testapi.TestResult)
		if !ok {
			debugLogger.Warn().Err(fmt.Errorf("invalid payload type: %T", input[i].GetPayload()))
			continue
		}
...
func getTotalNumberOfShimFindings(findings []*testapi.TestResult) uint32 {
	if findings == nil {
		return 0
	}
	...
	var count uint32
	for i := range findings {
		count = count + findings[i].GetEffectiveSummary().Count
	}
	return count
}
...
func useRendererWithShim(name string, wEntry *WriterEntry, findings []*testapi.TestResult, invocation workflow.InvocationContext) {
	debugLogger := invocation.GetEnhancedLogger()

	if !wEntry.renderEmptyData && getTotalNumberOfShimFindings(findings) == 0 {
		debugLogger.Info().Msgf("[%s] The input is empty, skipping rendering!", name)
		return
	}
...
func HandleContentTypeShimModel(input []workflow.Data, invocation workflow.InvocationContext, outputDestination iUtils.OutputDestination) ([]workflow.Data, error) {
	var err error
	debugLogger := invocation.GetEnhancedLogger()
	config := invocation.GetConfiguration()

	findings, remainingData := getListOfShimFindings(input, debugLogger)
	if len(findings) == 0 {
		debugLogger.Info().Msg("No findings to process")
		return input, nil
	}
