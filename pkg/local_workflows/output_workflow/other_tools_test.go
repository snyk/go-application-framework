...
func Test_HandleContentTypeOther(t *testing.T) {
	...
	t.Run("text/toon data is routed to the text/toon writer", func(t *testing.T) {
		ctx := newMockContext(t)
		buffer := &bytes.Buffer{}
		writers := &testWriterMap{
			writers: map[string][]*WriterEntry{
		...
		data := workflow.NewData(workflowID, content_type.TOON, []byte("issues[1]{id}:\n  SNYK-1"))
		remaining, err := HandleContentTypeOther([]workflow.Data{data}, ctx, writers)
		assert.NoError(t, err)
		assert.Equal(t, "issues[1]{id}:\n  SNYK-1", buffer.String())
		assert.Empty(t, remaining)
	})
