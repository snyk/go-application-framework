...
func initConfiguration(engine workflow.Engine, config configuration.Configuration, logger *zerolog.Logger, apiClientFactory func(url string, client *http.Client) api.ApiClient) {
	...
	config.AddDefaultValue(configuration.NETWORK_REQUEST_RETRY_EXCLUDED_PATH_SEGMENTS, configuration.StandardDefaultValueFunction(constants.DEFAULT_RETRY_EXCLUDED_PATH_SEGMENTS))
