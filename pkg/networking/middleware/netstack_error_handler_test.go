...
func Test_NetworkStackErrorHandlerMiddleware(t *testing.T) {
	...
	t.Run("handleError with regular Go error - converts to generic CLI failure error", func(t *testing.T) {
		...
		assert.Contains(t, cliError.Detail, "http://example.com/test")
		assert.Contains(t, cliError.Detail, "regular network error")
		assert.Contains(t, cliError.Detail, "Error Type: Network Error")
	})
...
func Test_categorizeNetworkError(t *testing.T) {
	...
	t.Run("DNS Resolution Error - net.DNSError", func(t *testing.T) {
		...
		// Verify it returns a generic CLI failure error
		var cliError snyk_errors.Error
		assert.True(t, errors.As(result, &cliError))

		expectedGenericError := cli.NewGeneralCLIFailureError("")
		assert.Equal(t, expectedGenericError.ErrorCode, cliError.ErrorCode)
		assert.Contains(t, cliError.Detail, "Error Type: DNS Resolution Error")
		assert.Contains(t, cliError.Detail, "https://api.snyk.io/test")
		assert.Contains(t, cliError.Detail, "no such host")
	})
	...
	t.Run("Connection Timeout - os.IsTimeout", func(t *testing.T) {
		...
		var cliError snyk_errors.Error
		assert.True(t, errors.As(result, &cliError))
		assert.Contains(t, cliError.Detail, "Error Type: Connection Timeout")
	})
	...
	t.Run("Connection Refused - net.OpError", func(t *testing.T) {
		...
		var cliError snyk_errors.Error
		assert.True(t, errors.As(result, &cliError))
		assert.Contains(t, cliError.Detail, "Error Type: Connection Refused")
	})
	...
	t.Run("Network Unreachable - net.OpError", func(t *testing.T) {
		...
		var cliError snyk_errors.Error
		assert.True(t, errors.As(result, &cliError))
		assert.Contains(t, cliError.Detail, "Error Type: Network Unreachable")
	})
	...
	t.Run("Proxy unreachable - should not be TLS error", func(t *testing.T) {
		...
		assert.True(t, errors.As(result, &cliError))
		assert.Contains(t, cliError.Detail, "Error Type: Network Unreachable")
		assert.NotContains(t, cliError.Detail, "Error Type: TLS/SSL Certificate Error")
	})
	...
	t.Run("TLS Alert Error", func(t *testing.T) {
		...
		var cliError snyk_errors.Error
		assert.True(t, errors.As(result, &cliError))
		assert.Contains(t, cliError.Detail, "Error Type: TLS/SSL Certificate Error")
	})
	...
	t.Run("Complex error precedence - DNS should win over timeout", func(t *testing.T) {
		...
		assert.True(t, errors.As(result, &cliError))
		assert.Contains(t, cliError.Detail, "Error Type: DNS Resolution Error")
		assert.NotContains(t, cliError.Detail, "Error Type: Connection Timeout")
	})
