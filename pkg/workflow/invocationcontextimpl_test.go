...
func TestInvocationContextImpl_GetFileFilter(t *testing.T) {
	...
	t.Run("reports filter metrics through the analytics of the invocation", func(t *testing.T) {
		...
		for key := range *obj.Data.Attributes.Interaction.Extension {
			// keys are scoped per FileFilter instance: file-filter.<scopeID>.<metric>
			if prefix, metric, found := strings.Cut(key, "."); found && prefix == "file-filter" {
				recorded = append(recorded, metric[strings.Index(metric, ".")+1:])
			}
		}
		assert.ElementsMatch(t, []string{"durationMs", "fileCount", "metacharacterFix", "respectTrackedFiles"}, recorded)
	})
}
