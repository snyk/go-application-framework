...
const (
	...
	NETWORK_REQUEST_RETRY_ALLOWED_PATHS string = "internal_network_request_retry_allowed_paths" // NETWORK_REQUEST_RETRY_ALLOWED_PATHS ([]string) URL paths eligible for GAF's network retries on unsafe HTTP methods; safe methods always retry regardless of this list, an unsafe method retries only on a path match; the framework ships no paths, each application declares its own
