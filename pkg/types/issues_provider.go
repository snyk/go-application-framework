...
type IssueProvider interface {
	IssuesForFile(path FilePath) []Issue
	IssuesForRange(path FilePath, r Range) []Issue
...
type CacheProvider interface {
	...
	Clear()
	ClearIssues(path FilePath)
	RegisterCacheRemovalHandler(handler func(path FilePath))
}
