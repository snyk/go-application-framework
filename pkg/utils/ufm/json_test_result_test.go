...
func TestNewSerializableTestResult(t *testing.T) {
	...
	mock.EXPECT().Get(testapi.TestResultTestSubject).Return(nil).AnyTimes()
	mock.EXPECT().Get(testapi.TestResultSubjectLocators).Return(nil).AnyTimes()
	mock.EXPECT().Get(testapi.TestResultBreachedPolicies).Return(nil).AnyTimes()
	mock.EXPECT().Get(testapi.TestResultRawSummary).Return(nil).AnyTimes()
	mock.EXPECT().Get(testapi.TestResultTestFacts).Return(nil).AnyTimes()
	mock.EXPECT().Get(testapi.TestResultMetadata).Return(nil).AnyTimes()
	mock.EXPECT().Get(testapi.TestResultComponents).Return(nil).AnyTimes()
...
func Test_Findings_ConcurrentAccess(t *testing.T) {
	...
	for i := 0; i < 10; i++ {
		go func() {
			...
			require.NoError(t, err)
			require.True(t, complete)
			require.Len(t, findings, len(findings))
