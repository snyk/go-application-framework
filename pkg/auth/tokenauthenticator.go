...
var UnsupportedPatRegions = []string{
...
// DeriveEndpointFromPAT iterates a list of Snyk regions and tries to make an authenticated request to the Snyk PAT API
// on success, it will return the correct Snyk endpoint from the given PAT
func DeriveEndpointFromPat(pat string, config configuration.Configuration, client *http.Client, regions []string) (string, error) {
	var (
		err      error
		errs     error
		endpoint string
	...
	supportedPatRegions := FilterSupportedPatRegions(regions, UnsupportedPatRegions)
	for _, region := range ShuffleStrings(supportedPatRegions) {
		endpoint, err = deriveEndpoint(pat, config, client, region)
		if err != nil {
