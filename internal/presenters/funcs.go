...
func getDefaultTemplateFuncMap(config configuration.Configuration, ri runtimeinfo.RuntimeInfo) template.FuncMap {
	...
	defaultMap["getFirstMetadataValue"] = getFirstMetadataValue
...
func getFirstMetadataValue(testResults []testapi.TestResult, key string) string {
	for _, result := range testResults {
		if result == nil {
			continue
		}
		if value, ok := result.GetMetadataValue(key).(string); ok && value != "" {
			return value
		}
	}
