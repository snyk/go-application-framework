# testapi-example-low-level script

This is a developer script that loads a dependency graph, runs a Snyk test on it, and displays the test findings. This version interacts with the low-level API.
...
This example interacts directly with the low-level test API, which requires more management than the high-level API. For most purposes the high-level API (demoed in /testapi-example-high-level) provides a simpler interface.
...
```
./scripts/testapi-example-low-level/cli.sh <path to package manifest> [additional cli options]
```
