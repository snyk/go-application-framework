...
func run() error { //nolint:gocyclo // chill it's a demo script
	...
	// Load a /v1/test-dep-graph json request body
	contents, err := os.ReadFile("scripts/local-client/testdata/req-dep-graph-test-npm.json")
	if err != nil {
		return fmt.Errorf("read test data failed: %w", err)
	}
	...
	ae := v20241015defs.AssetExprCreate{}
	err = ae.FromSnykDepGraphCreate(v20241015defs.SnykDepGraphCreate{
		AssetId:     assetID,
		DepGraph:    depGraphStruct.DepGraph,
		SourceFiles: []string{"package.json"},
		Type:        v20241015defs.SnykDepGraphCreateType("snyk_dep_graph"),
	})
	if err != nil {
